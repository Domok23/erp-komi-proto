...
@section('content')
    <div class="mb-8">
        ...
        <h1 class="text-2xl sm:text-3xl font-extrabold text-slate-900 dark:text-white tracking-tight">Employee Leave Request</h1>
        <p class="text-sm text-slate-600 dark:text-slate-400 mt-1">Enter your Employee Number (NIK) to check available balance and submit a leave request.</p>
    </div>
    ...
    <form method="POST" action="{{ route('leave-request.lookup.submit', $company->code) }}" class="space-y-6">
        ...
        <div>
            ...
            <div class="relative">
                <input type="text" name="employee_number" placeholder="e.g. EMP-101" class="w-full border border-slate-300 dark:border-slate-700 rounded-2xl px-4 py-3.5 pl-11 text-base bg-slate-50 dark:bg-slate-950/80 text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-500 focus:ring-2 focus:ring-amber-500 focus:border-amber-500 focus:outline-none transition shadow-2xs font-mono font-medium min-h-[52px]" required>
